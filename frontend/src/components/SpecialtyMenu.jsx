    ...
    <div className='flex flex-col items-center gap-4 py-16' id='specialty'>
        ...
        <div className='flex sm:justify-between gap-4 pt-5 px-10 w-full overflow-scroll'>
            {specialtyData.map((item, i) => (
                <Link onClick={() => scrollTo(0,0)} className='flex flex-col items-center text-xs cursor-pointer flex-shrink-0 hover:translate-y-[-10px] transition-all duration-500' key={i} to={`/doctors/${item.specialty}`}>
